...
//     it('should allow a user to register, login, create a recipe, and then delete it', async () => {
...
//         // 3. Create a new recipe using the auth token
...
//         recipeId = createRecipeRes.body._id; // Save the recipe ID

//         // 4. Verify the recipe can be fetched
//         const getRecipeRes = await request(app).get(`/api/recipes/${recipeId}`);
//         expect(getRecipeRes.statusCode).toBe(200);
//         expect(getRecipeRes.body.title).toBe('E2E Test Recipe');

//         // 5. Delete the recipe using the auth token
